...
function isPalindrome(str) {
  const charsArray = str.toLowerCase().split('');
  const lettersOnly = charsArray.filter((char) => char >= 'a' && char <= 'z');
  const cleanedStr = lettersOnly.join('');
  return cleanedStr === lettersOnly.reverse().join('');
}
...
function extractNameFromTemplate(value) {
  const startIndexOfName = value.indexOf('Hello, ') + 'Hello, '.length;
  const endIndexOfName = value.indexOf('!', startIndexOfName);
  return value.slice(startIndexOfName, endIndexOfName);
}
